 ...
 *
 * Options:
 *   --type <type>    Campaign type: podcast | product | newsletter | blog | security | hero | sonex | all
...
const CAMPAIGNS = {
  ...
  sonex: {
    label: 'SoNex Product Ad',
    prompts: [
      // LinkedIn banner (16:9)
      `Google-quality LinkedIn advertisement banner for a privacy-first Android smart-volume app called SoNex. Dark cinematic background — deep navy #0f172a. Center: abstract 3D soundwave that smoothly ducks and rises, rendered in electric violet gradient, with concentric rings radiating outward like a room listening. Left side: clean headline area in white bold sans-serif "SMART VOLUME. ZERO EFFORT." Subtle microphone icon suggestion. Bottom strip: domain text area "sonex.patienceai.in" in small caps violet. Professional tech product ad. No people. ${STYLE_SUFFIX}`,

      // LinkedIn carousel / article image (16:9)
      `Sleek LinkedIn product feature card for SoNex — room-aware adaptive volume app. Split layout: left half dark navy with glowing on-device AI chip icon emitting audio waves in violet/white. Right half: matte slate panel with bold headline "YOUR ROOM. YOUR VOLUME." and three icon+text rows: 🔇 Ducks when you talk  •  🔊 Rises in noise  •  🔒 On-device privacy. Inspired by Apple product ad aesthetic, premium, minimal. ${STYLE_SUFFIX}`,

      // Instagram square (1:1) — use with --aspect 1:1
      `Instagram square ad for SoNex smart-volume Android app. Bold center-stage composition. Deep navy background. Massive abstract ear/speaker icon made of flowing violet light-lines, slightly off-center. Bold white headline at top "Volume that thinks." Subtle floating sound-ripple rings in very dark violet behind the icon. Bottom: product name "SoNex" in large clean white type. Punchy, scroll-stopping, premium Gen-Z aesthetic. No people. ${STYLE_SUFFIX}`,

      // Instagram Story / Reel cover (9:16) — use with --aspect 9:16
      `Instagram Story ad for SoNex. Vertical 9:16 format. Full-bleed deep navy background. Top third: large bold white text "SoNex" + tagline "Room-aware volume. On-device AI." Center: dramatic abstract visualization — a living room silhouette outline in very dim navy, with a glowing violet soundwave emanating from a phone shape, automatically lowering as a speech-bubble icon appears (conversation detected). Bottom: CTA strip "Try it → sonex.patienceai.in" in violet rounded pill shape. Cinematic vertical billboard quality. ${STYLE_SUFFIX}`,
    ],
  },
...
Options:
  --type <type>    Campaign: podcast | product | newsletter | blog | security | hero | sonex | all
  --count <n>      Images per prompt: 1-4  (default: 1)
  --aspect <r>     Aspect ratio: 16:9 | 1:1 | 4:3 | 9:16  (default: 16:9)
  --out <dir>      Output directory  (default: public/generated-ads)
...
Campaign types:
  ...
  sonex       — SoNex product ads for LinkedIn & Instagram
  all         — Generate every campaign type

SoNex tip — use aspect ratio per platform:
  LinkedIn banner / post  : --aspect 16:9  (prompts 1 & 2)
  Instagram square        : --aspect 1:1   (prompt 3)
  Instagram Story / Reel  : --aspect 9:16  (prompt 4)
`);
  process.exit(0);
}
